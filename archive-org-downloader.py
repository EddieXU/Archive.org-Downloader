...
if __name__ == "__main__":
    ...
    for url in urls:
        if list(filter(None, url.split("/")))[-3]== 'details' :    # meaning the url is of long format (book_id/volume_id)
            book_id = list(filter(None, url.split("/")))[3]+'/'+list(filter(None, url.split("/")))[4]
            book_id0 = list(filter(None, url.split("/")))[-2]
            session = loan(session, book_id0, False)  # must borrow the book_id0 first before borrow volume_id (don't know why)
        else:
